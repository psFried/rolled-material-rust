...
#[test]
fn test_parse() {
    let fifty_cm = units::parse_str("50", units::CENTIMETERS);
    assert!(fifty_cm.is_some());
    let len = fifty_cm.unwrap();
    assert_eq!(len.unit, units::CENTIMETERS);
    assert_equals(50.0f64, len.value, 0.0001f64);
}

#[test]
fn test_unit_conversion() {
    let len_inches = units::Length{value: 66.11f64, unit: units::INCHES};
    let len_cm = len_inches.convert_to(units::CENTIMETERS);
    let eps = 0.0001f64;
    let expected = 167.9194f64;
    println!("length in cm= {}, in meters= {}", len_cm.value, len_inches.convert_to(units::METERS).value);
    assert_equals(expected, len_cm.value, eps);
}

#[test]
fn test_length_format() {
    let len = units::Length{value: 40.0f64, unit: units::INCHES};

    assert_eq!("40.00in".to_string(), len.format());

    let len2 = units::Length{value: 77.333333333333f64, unit: units::METERS };
